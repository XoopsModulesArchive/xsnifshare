...
if (!$xoopsUser){
 $url = "browse.php?guest=true"; $delay = "0";
echo '<meta http-equiv="refresh" content="'.$delay.';url='.$url.'">';
}
$xsnif_name = explode("/", $_SERVER['SCRIPT_NAME']);
$xsnif_db = $xsnif_name[2] ."_data";
$size_path=XOOPS_ROOT_PATH."/modules/". $xsnif_name[2] ."/". $xoopsUser->getVar('uname') ;
$ar=getDirectorySize($size_path);
if (is_dir($xoopsUser->getVar('uname')) === FALSE){
mkdir($xoopsUser->getVar('uname'), 0755);
$lockfilename = $xoopsUser->getVar('uname') ."/.locked";
copy("admin/.htaccess", $lockfilename);
$path = $xoopsUser->getVar('uname') ."/";
}
if (($_GET["path"] != $xoopsUser->getVar('uname') ."/") && !$xoopsUser->isAdmin(-1) ){
$up_check = explode("/", $_GET["path"]);
if ($up_check[0] != $xoopsUser->getVar('uname')){
$path = $xoopsUser->getVar('uname') ."/";
}
}
...
while($row = mysql_fetch_array($result))
  ...
  $banned_uploads = explode("|", $row['banned_uploads']);
...
if (!$allowSubDirs) {
	...
	if ($path != $xoopsUser->getVar('uname') ."/"){
	$path = $_GET["path"];
...
if ($_GET["remove"]!="") {
   $delete = stripslashes($_GET["remove"]);
   $filename = safeDirectory($path.rawurldecode($delete));
   if (
      !file_exists($filename)
      OR fileIsHidden($filename)
      OR (substr(strtolower($filename), -4)==".php" AND !$allowPHPDownloads)) {
      ...
      unlink($filename);
      $url = $_SERVER['PHP_SELF'] . "?path=" . urlencode($path);
      echo '<meta http-equiv="refresh" content="0;url='.$url.'">';
      die();
   }
}

// handle removedir requests
if ($_GET["removedir"]!="") {
   $delete = stripslashes($_GET["removedir"]);
   $filename = safeDirectory($path.rawurldecode($delete));
   if (
      !file_exists($filename)
      OR fileIsHidden($filename)
			...
			switch ($column) {
				...
				case "delete":
                    ...
                    if ($files[$i]["isDirectory"]==false){
                                    ?>
                     <a href="<?php echo $PHP_SELF; ?>?path=<?php echo rawurlencode($path); ?>&amp;remove=<?php echo rawurlencode($files[$i]["name"]); ?>">x</a>
                     <?php
               }
               if ($files[$i]["isDirectory"]==true AND $files[$i]["displayName"] != "[ back ]"){
                                    ?>
                     <a href="<?php echo $PHP_SELF; ?>?path=<?php echo rawurlencode($path); ?>&amp;removedir=<?php echo rawurlencode($files[$i]["name"]); ?>">x</a>
                     <?php
               }
               echo "</td>";
               break; 
...
<?php
$user_space_limit_mb = $user_space;//in MB
$user_space_limit = ($user_space_limit_mb * 1048576);
$user_avail_space = $user_space_limit - $ar['size'];
$lockfilename = XOOPS_ROOT_PATH."/modules/". $xsnif_name[2] ."/". $xoopsUser->getVar('uname') ."/.locked";
if (file_exists($lockfilename) && $admin_activation == "0"){
unlink($lockfilename);
}
if (file_exists($lockfilename)){
echo "Your upload account has not yet been approved by admin";
}
if (!file_exists($lockfilename)){
if ($ar['size'] < $user_space_limit){
// Convert current dir to realdir
$realdir = dirname($_SERVER['SCRIPT_FILENAME'])."/".$path;
$last_char = substr($realdir, strlen($realdir)-1, 1);
if ($last_char != '/') {
   $realdir = $realdir . "/";
}
...
?>
<form method="post" action="<?php $_SERVER['PHP_SELF'] . "?path=" . $path;?>">
<p>
 <input type="text" name="createdir" size="20" />
...
$createdir = str_replace("\x2f", "", $createdir); // removes /
$createdir = str_replace("\x5c", "", $createdir); // removes \
$createdir = utf8_decode($createdir);
$DirExist = FALSE;

 if ($createdir != ""){
   if (!file_exists($realdir.$createdir)) {
      mkdir($realdir.$createdir, 0755);
      $url = $_SERVER['PHP_SELF'] . "?path=" . urlencode($path) . urlencode($createdir . "/") . "&DirOK=" . urlencode($createdir);
      echo '<meta http-equiv="refresh" content="0;url='.$url.'">';
   }else{
      $DirExist = TRUE;
   }
...
$file = $_GET['file'];
if ($file=="1"){
echo "Upload Successful! Care to upload another?";
}
if ($file=="2") {
echo "Upload Failed, please try again. If the problem persists, please contact the system administrator.";
}
if ($file=="3"){
echo "Upload Failed, you are trying to upload a file with a bad extension";
}
if ($file=="4") {
echo "Upload Failed, you have used your available space please delete some files if you wish to upload this file or try uploading a smaller file.";
}
...
// Do NOT allow upload for following file types:
$denyupload = $banned_uploads; 
$file_ext   = strtolower(substr($file_name, strrpos($file_name, '.')+1));
 	if ($_FILES['uploadFile'. $x]['size'] <= $user_avail_space) {
    if (!in_array(strtolower($file_ext), $denyupload)) { 

$copy = copy($_FILES['uploadFile'. $x]['tmp_name'],$file_name);
 // check if successfully copied
 if($copy){
$url = $_SERVER['PHP_SELF'] . "?path=" . $_GET['path'] ."&file=1"; $delay = "0";
echo '<meta http-equiv="refresh" content="'.$delay.';url='.$url.'">';
 }else{
$url = $_SERVER['PHP_SELF'] . "?path=" . $_GET['path'] ."&file=2"; $delay = "0";
echo '<meta http-equiv="refresh" content="'.$delay.';url='.$url.'">';
 }
 }else{
 $url = $_SERVER['PHP_SELF'] . "?path=" . $_GET['path'] ."&file=3"; $delay = "0";
echo '<meta http-equiv="refresh" content="'.$delay.';url='.$url.'">';
 }
 }else{
  $url = $_SERVER['PHP_SELF'] . "?path=" . $_GET['path'] ."&file=4"; $delay = "0";
echo '<meta http-equiv="refresh" content="'.$delay.';url='.$url.'">';
 }
} // end of loop
...
?>
<br>
<form name="form1" enctype="multipart/form-data" method="post" action="<?php $_SERVER['PHP_SELF'] . "?path=" . $path;?>">
    <?
  // start of dynamic form
  
  if (isset($_POST['munum'])){
  $uploadNeed = $_POST['munum'];
  }else{
  $uploadNeed = "1";
  }
  for($x=0;$x<$uploadNeed;$x++){
...
?>
  ...
  <input name="uploadNeed" type="hidden" value="<? echo $uploadNeed;?>">
    <input type="submit" name="Submit" value="Upload File(s)"></center><br>
    Select file(s) to upload or select number of files you wish to upload below for multiple files..

</form>
<form name="formmu" method="post" action="<?php $_SERVER['PHP_SELF'] . "?path=" . $path;?>">
<select name="munum">
...
echo "<h2>Info for : ". $xoopsUser->getVar('uname') ."</h2>";
echo "Total space : ".sizeFormat($user_space_limit)."<br>";
echo "Space used : ".sizeFormat($ar['size'])."<br>";
echo "Space available : ".sizeFormat($user_avail_space)."<br>";
echo "No. of files : ".$ar['count']."<br>";
echo "No. of directories : ".$ar['dircount']."<br>"; 
//if admin
if($xoopsUser->isAdmin(-1)){
?>
<a href="index.php?path=<? echo $_GET['path']; ?>&listall=true">List All Files</a><br />
<?
$lockfilename = XOOPS_ROOT_PATH."/modules/". $xsnif_name[2] ."/". $_GET['path'] .".locked";
if (file_exists($lockfilename)){
?>
<a href="index.php?path=<? echo $_GET['path']; ?>&unlock=<? echo $_GET['path']; ?>"><font color="#FF0000">UNLOCK USER</font></a><br />
<?
}
if (isset($_GET['unlock'])){
unlink($lockfilename);
 $url = $_SERVER['PHP_SELF'] . "?path=" . $_GET['path']; $delay = "0";
echo '<meta http-equiv="refresh" content="'.$delay.';url='.$url.'">';
}
if (isset($_GET['admin_delete'])){
$admin_file_check = explode("/", str_replace("./", "", $_GET['admin_delete']));
$fname = str_replace("./", "", $_GET['admin_delete']);
if (($admin_file_check[0] != "admin") && ($fname != "xoops_version.php" && "index.php" && "browse.php")){
unlink(str_replace("./", "", $_GET['admin_delete']));
echo "<font color=\"#FF0000\">". $_GET['admin_delete'] ." has been deleted</font>.";
}else{
echo "<font color=\"#FF0000\">This is a system file you can not delete it</font>";
}
} 
if ($_GET['listall'] == true){
...
if (!is_dir($filename))
printf("<a href=\"%s\">%s</a> <a href=\"index.php?admin_delete=%s\"><font color=\"#FF0000\">DELETE</font></a><br>\n",$filename,$filename,$filename);
// otherwise tell the user it's a "category"

else printf("<p>Directory: %s<p>\n",$filename);
}
}
}
?>
...
<br/>
<a href="http://badcase.net">
X.S.N.I.F. Share 1.0
&copy; 2009 Nathan "BadCase" Fluet</a><br/>
